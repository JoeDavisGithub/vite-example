...
const CreateMoviePage = () => {
    ...
    const handleFSubmit = (e:React.FormEvent<HTMLFormElement>)=>{
        ...
        fetch(API_URL+'/films', {
            ...
            setIsPending(false);
            fetch(API_URL+'/films?title='+title)
            .then((response) => response.json())
            .then((data) => { navigate("/movie?title="+data[0].id)})
        })
        //
    }
        ...
        <>
            ...
            <form className="createform" onSubmit={handleFSubmit}>
                ...
                <div className="formtexts">
                    ...
                    <div>
                        <input className="createmovinp" type="text" name="movtitle" required onChange={(e) => setTitle(e.target.value)} ></input><br />
                        <input className="createmovinp" type="text" name="movdescription" required onChange={(e) => setDescription(e.target.value)} ></input><br />
                        <input className="createmovinp" type="Number" name="movyear" min={1901} max={2155}  onChange={(e) => setRelease_Year(Number(e.target.value))} ></input><br />
                        <input className="createmovinp" type="Number" required name="movrentalrate" min={1}  onChange={(e) => setRental_Rate(Number(e.target.value))} ></input><br />
                        <input className="createmovinp" type="Number" required name="movrentalduration" min={1} max={20}  onChange={(e) => setRental_Duration(Number(e.target.value))} ></input><br />
                        <input className="createmovinp" type="Number" required name="movreplacementcost" min={1}  onChange={(e) => setReplacement_Cost(Number(e.target.value))} ></input><br />
                        <input className="createmovinp" type="Number" name="movlength" min={1} onChange={(e) => setLength(Number(e.target.value))} ></input><br />
                        <select name="selectRating" defaultValue={"G"} onChange={(e)=> setRating(e.target.value)}>
                        ...
                        <ul>
                            <li>
                                ...
                                <input type="checkbox" name="Trailers" value="Trailers" onChange={(e)=> {handleCheckbox(e.target.value)}}/>
                            </li>
                            <li>
                                <label >Commentaries</label>
                                <input type="checkbox"  name="Commentaries" value="Commentaries" onChange={(e)=> {handleCheckbox(e.target.value)}}/>
                            </li>
                            <li>
                                <label >Deleted Scenes</label>
                                <input type="checkbox" name="Deleted Scenes" value="Deleted Scenes" onChange={(e)=> {handleCheckbox(e.target.value)}}/>
                            </li>
                            <li>
                                <label  >Behind the Scenes</label>
                                <input type="checkbox" name="Behind the Scenes" value="Behind the Scenes" onChange={(e)=> {handleCheckbox(e.target.value)}}/>
                            </li>
                        </ul><br />
